...
const focusInput = (target, checkVal) => {
  ...
  if (target.value === checkVal) target.value = "";
};
...
const setActiveTipButton = (target, setTipRate) => {
  ...
  if (activeEl !== target) {
    ...
    if (target.tagName === "INPUT") setTipRate(0);
  }
};
...
const Calculator = (props) => {
  ...
  const sum = total.reduce((acc, crr) => {
    return acc + crr.item.price;
  }, 0);
  const [billAmount, setBillAmount] = useState(sum);
  ...
  const funcCustomTipRate = (val) => {
    ...
    if (
      val <= 100 &&
      (val === "" || checkPattern(val, "^[1-9]*[0-9]\\.?[0-9]{0,2}$"))
    ) {
    ...
    <div className="box row justify-content-between mx-auto mt-md-3 p-4 p-lg-3 col-12 col-sm-9 col-md-7 col-lg-9 col-xl-7 shadow">
      <div className="col-lg-6 py-2 px-0 px-lg-3 d-flex flex-column justify-content-between">
        <div className="mb-4 mb-md-3">
          <label className="fw-bold mb-1 text-capitalize">Amount</label>
          <div className="input-group border border-2 border-white rounded-3">
            <span className="input-group-text border-0 bg-transparent">
              {/* <img src={process.env.PUBLIC_URL + "/icon-dollar.svg"} alt="KD" /> */}
            </span>
            <input
              type="text"
              className="form-control py-1 border-0 text-end fw-bold bg-transparent shadow-none"
              aria-label="Total Amount"
              onFocus={(event) => {
                focusInput(event.target, "0");
              }}
              onBlur={(event) => blurInput(event.target, "0")}
              onChange={(event) => {
                funcBillAmount(event.target.value.trim());
              }}
              value={billAmount}
            />
          </div>
        ...
        {/* <div className="mb-4 mb-md-3">
          ...
          <div className="tip-btn-grp row justify-content-between gx-2 mt-2">
            {tipRates.map((tip, ind) => {
              return (
                <div className="col-4 my-1" key={ind}>
                  <button
                    className={
                      "btn fw-bold py-1 w-100 border border-2 border-white rounded-3 shadow-none" +
                      (tip === tipRate ? " active" : "")
                    }
                    onClick={(event) => {
                      funcTipRate(event.target, tip);
                    }}
                  >
                    {tip + "%"}
                  </button>
            ...
            <div className="col-4 my-1">
              <input
                type="text"
                className="form-control py-1 border-2 border-white rounded-3 text-end fw-bold shadow-none"
                aria-label="Custom Tip Rate"
                onBlur={(event) => {
                  checRate(event.target.value);
                }}
                onClick={(event) => {
                  setActiveTipButton(event.target, setTipRate);
                }}
                onChange={(event) => {
                  funcCustomTipRate(event.target.value);
                }}
                value={customTipRate}
                placeholder="Custom"
              />
        ...
        <div className="">
          <label className="fw-bold mb-1 text-capitalize">
            number of people
          </label>
          <div className="input-group border border-2 border-white rounded-3">
            <span className="input-group-text border-0 bg-transparent">
              <img
                src={process.env.PUBLIC_URL + "/icon-person.svg"}
                alt="people"
              />
            </span>
            <input
              ...
              onFocus={(event) => {
                focusInput(event.target, "1");
              }}
              onBlur={(event) => {
                blurInput(event.target, "1");
              }}
              onChange={(event) => {
                funcPeopleCount(event.target.value.trim());
              }}
              value={peopleCount}
            />
          </div>
      ...
      <div className="col-lg-custom p-4 d-flex flex-column justify-content-between">
        <div>
          {/* <div className="mx-2 mb-3 mb-md-2 mt-2 d-flex justify-content-between align-items-center">
            <label>
              <span className="text-capitalize">tip amount</span>
              <br />
              <span>/ person</span>
            </label>
            <span className="money fs-1 fw-bold">{"KD" + tipAmount}</span>
          </div> */}
          ...
          <div className="mx-2 my-3 my-md-2 d-flex justify-content-between align-items-center">
            ...
            <span className="money fs-1 fw-bold">{"KD" + totalBill}</span>
          </div>
        </div>
        <button
          className="btn btn-reset fw-bold shadow-none text-uppercase d-block mb-2 mt-3 mt-md-4 mt-lg-5"
          // onClick={resetAll}
          style={{ color: "white" }}
        >
          Share Link
